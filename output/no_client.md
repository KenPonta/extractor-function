...
- All countries show an upward trend in "lny" from 2000 to 2025.
- Singapore and Brunei consistently have the highest "lny" values (around 11–12).
- Cambodia has the lowest "lny" values but rises from about 7 to over 8 during the period.
- Other countries have intermediate values, generally increasing over time, with some fluctuations.
...
**X-axis:**
Alpha value (range: approximately -2.5 to +2.5)

**Data/Trends:**
- All countries show two overlapping horizontal bars for each country; one for Estimated α and one for Calibrated α.
- Most bars are positive, except for Brunei, where both values are negative.
- Vietnam has the highest positive α values, followed by Cambodia and Indonesia.
- Brunei has the most negative α values, distinctly separate from the others.
- The separation between Estimated α and Calibrated α for each country is small, making the bars nearly overlap.
...
Trends/Relationships:
- BRN (light blue) and SGP (dark blue) show the highest log TFP throughout the period, with SGP being consistently at the top.
- MYS (purple) follows as the next highest trajectory, steadily increasing over time.
- Other countries show lower log TFP values; most exhibit a gradually increasing trend from 2000 to 2025.
- KHM (teal/green) and LAO (yellow) start near the bottom but show some upward movement.
- General trend: steady, modest increases in log TFP across most countries, except for SGP and BRN, which start at higher values.
...
Legend/Series:
- BRN (Brunei) - blue line
- IDN (Indonesia) - dark red line
- KHM (Cambodia) - green line
- LAO (Laos) - yellow line
- MYS (Malaysia) - purple line
- PHL (Philippines) - orange line
- SGP (Singapore) - light blue line
- THA (Thailand) - brown line
- VNM (Vietnam) - teal line

Trends/Relationships Conveyed:
- Singapore (SGP) and Brunei (BRN) have the highest Log TFP throughout the time period.
- Singapore (SGP) remains consistently highest, though with some fluctuations.
- Malaysia (MYS) is intermediate and shows a slight upward trend.
- Thailand (THA), Philippines (PHL), and Indonesia (IDN) are clustered close together in the mid-range, trending gently upward.
- Vietnam (VNM), Laos (LAO), and Cambodia (KHM) start lower and trend upward gradually, with Vietnam (VNM) increasing steadily.
- Overall, most countries exhibit upward trajectories in Log TFP between 2000 and 2025, with Singapore and Brunei as clear leaders.
...
Trends/Relationships:
- The scatter plot shows a negative relationship: as the Initial TFP (2000) increases, the Cumulative TFP Growth (2000-2023) tends to decrease.
